...
public static class ImGuiDX11Impl
{
    ...
    // Functions
    private static unsafe void ImGui_ImplDX11_SetupRenderState(
        ImDrawDataPtr draw_data,
        IntPtr ID3D11DeviceContextPtr
    )
    {
        ...
        deviceContext.InputAssembler.SetIndexBuffer(_indexBuffer, Format.R16_UInt, 0);
        ...
        deviceContext.OutputMerger.SetDepthStencilState(_depthStencilState);
    ...
    public static unsafe void RenderDrawData(ImDrawDataPtr draw_data)
    {
        ...
        ctx.MapSubresource(
            ...
            MapFlags.None,
            out var vtx_resource
        );
        ctx.MapSubresource(_indexBuffer, MapMode.WriteDiscard, MapFlags.None, out var idx_resource);

        var vtx_dst = (ImDrawVert*)vtx_resource.DataPointer;
        var idx_dst = (ushort*)idx_resource.DataPointer;
        ...
        ctx.MapSubresource(
            ...
            MapMode.WriteDiscard,
            MapFlags.None,
            out var mapped_resource
        );

        var constant_buffer = (VERTEX_CONSTANT_BUFFER_DX11*)mapped_resource.DataPointer;
        ...
        for (var n = 0; n < draw_data.CmdListsCount; n++)
        {
            ...
            for (var cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++)
            {
                ...
                if (pcmd.UserCallback != ImDrawCmdPtr.Null)
                {
                    var userCallback = Marshal.GetDelegateForFunctionPointer<ImDrawUserCallBack>(
                        (IntPtr)pcmd.UserCallback
                    );

                    // User callback, registered via ImDrawList::AddCallback()
                    // (ImDrawCallback_ResetRenderState is a special callback value used by the user to request the renderer to reset render state.)
                    if ((IntPtr)pcmd.UserCallback == new IntPtr(-1))
                        ImGui_ImplDX11_SetupRenderState(draw_data, ctx.NativePointer);
                    else
                ...
                else
                {
                    ...
                    ctx.PixelShader.SetShaderResource(
                        0,
                        new ShaderResourceView((IntPtr)pcmd.TextureId.Handle)
                    );
                    ctx.DrawIndexed(
                        (int)pcmd.ElemCount,
                        (int)(pcmd.IdxOffset + global_idx_offset),
                        (int)(pcmd.VtxOffset + global_vtx_offset)
                    );
                }
            }
    ...
    private static StateBackup BackupRenderState(DeviceContext ctx)
    {
        ...
        ctx.InputAssembler.GetIndexBuffer(
            out backup.IndexBuffer,
            out backup.IndexBufferFormat,
            out backup.IndexBufferOffset
        );
        backup.PrimitiveTopology = ctx.InputAssembler.PrimitiveTopology;
        ctx.InputAssembler.GetVertexBuffers(
            0,
            InputAssemblerStage.VertexInputResourceSlotCount,
            backup.VertexBuffers,
            backup.VertexBufferStrides,
            backup.VertexBufferOffsets
        );

        // RS
        backup.RS = ctx.Rasterizer.State;
        ctx.Rasterizer.GetScissorRectangles(backup.ScissorRects);
        ctx.Rasterizer.GetViewports(backup.Viewports);

        // OM
        backup.BlendState = ctx.OutputMerger.GetBlendState(
            out backup.BlendFactor,
            out backup.SampleMask
        );
        backup.DepthStencilState = ctx.OutputMerger.GetDepthStencilState(
            out backup.DepthStencilRef
        );
        ...
        backup.VSConstantBuffers = ctx.VertexShader.GetConstantBuffers(
            0,
            CommonShaderStage.ConstantBufferApiSlotCount
        );
        backup.VSResourceViews = ctx.VertexShader.GetShaderResources(
            0,
        ...
        backup.HSConstantBuffers = ctx.HullShader.GetConstantBuffers(
            0,
            CommonShaderStage.ConstantBufferApiSlotCount
        );
        backup.HSResourceViews = ctx.HullShader.GetShaderResources(
            0,
            CommonShaderStage.InputResourceSlotCount
        );
        ...
        backup.DSConstantBuffers = ctx.DomainShader.GetConstantBuffers(
            0,
            CommonShaderStage.ConstantBufferApiSlotCount
        );
        backup.DSResourceViews = ctx.DomainShader.GetShaderResources(
            0,
            CommonShaderStage.InputResourceSlotCount
        );
        ...
        backup.GSConstantBuffers = ctx.GeometryShader.GetConstantBuffers(
            0,
            CommonShaderStage.ConstantBufferApiSlotCount
        );
        backup.GSResourceViews = ctx.GeometryShader.GetShaderResources(
            0,
            CommonShaderStage.InputResourceSlotCount
        );
        ...
        backup.PSConstantBuffers = ctx.PixelShader.GetConstantBuffers(
            0,
            CommonShaderStage.ConstantBufferApiSlotCount
        );
        backup.PSResourceViews = ctx.PixelShader.GetShaderResources(
            0,
            CommonShaderStage.InputResourceSlotCount
        );
        ...
        backup.CSConstantBuffers = ctx.ComputeShader.GetConstantBuffers(
            0,
            CommonShaderStage.ConstantBufferApiSlotCount
        );
        backup.CSResourceViews = ctx.ComputeShader.GetShaderResources(
            0,
            CommonShaderStage.InputResourceSlotCount
        );
        backup.CSUAVs = ctx.ComputeShader.GetUnorderedAccessViews(
            0,
            ComputeShaderStage.UnorderedAccessViewSlotCount
        ); // should be register count and not slot, but the value is correct
    ...
    private static void RestoreRenderState(DeviceContext ctx, StateBackup old)
    {
        ...
        ctx.InputAssembler.SetIndexBuffer(
            old.IndexBuffer,
        ...
        ctx.InputAssembler.SetVertexBuffers(
            0,
            old.VertexBuffers,
            old.VertexBufferStrides,
            old.VertexBufferOffsets
        );
    ...
    public static unsafe void CreateFontsTexture()
    {
        ...
        using (
            var fontTexture = new Texture2D(
                _device,
                texDesc,
                new DataRectangle((IntPtr)fontPixels, fontWidth * fontBytesPerPixel)
            )
        )
        {
            // Create texture view
            _fontResourceView = new ShaderResourceView(
                _device,
                fontTexture,
        ...
        io.Fonts.SetTexID(new ImTextureID(_fontResourceView.NativePointer));

        // Create texture sampler
        _fontSampler = new SamplerState(
            _device,
    ...
    public static bool CreateDeviceObjects()
    {
        ...
        var shadersFolder = Path.Combine(ImGuiInjector.AssetsFolderPath, "Shaders");
        ...
        _vertexConstantBuffer = new Buffer(
            _device,
            new BufferDescription
            {
                Usage = ResourceUsage.Dynamic,
        ...
        var pixelShaderBytes = File.ReadAllBytes(pixelShaderPath);
        _pixelShader = new PixelShader(_device, pixelShaderBytes);

        // Create the blending setup
        // ...of course this was setup in a way that can't be done inline
        var blendStateDesc = new BlendStateDescription { AlphaToCoverageEnable = false };
    ...
    public static void InvalidateDeviceObjects()
    {
        ...
        ImGui.GetIO().Fonts.SetTexID(ImTextureID.Null);
    ...
    internal static unsafe void Init(void* device, void* deviceContext)
    {
        ...
        ImGui.GetIO().Handle->BackendRendererName = (byte*)_renderNamePtr.ToPointer();
    ...
    private delegate void ImDrawUserCallBack(ImDrawListPtr a, ImDrawCmdPtr b);
