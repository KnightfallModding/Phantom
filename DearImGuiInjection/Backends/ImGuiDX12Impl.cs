...
using Range = SharpDX.Direct3D12.Range;
...
public static class ImGuiDX12Impl
{
    ...
    private static readonly RawVector4 _blendFactor = new(0, 0, 0, 0);
    ...
    // Functions
    private static unsafe void ImGui_ImplDX12_SetupRenderState(
        ImDrawDataPtr draw_data,
        GraphicsCommandList ctx,
        RenderBuffer fr
    )
    {
    ...
    // Render function
    public static unsafe void RenderDrawData(ImDrawDataPtr draw_data, GraphicsCommandList ctx)
    {
        ...
        if (fr.VertexBuffer == null || fr.VertexBufferSize < draw_data.TotalVtxCount)
        {
            ...
            fr.VertexBuffer = _device.CreateCommittedResource(
                props,
                HeapFlags.None,
                desc,
        ...
        if (fr.IndexBuffer == null || fr.IndexBufferSize < draw_data.TotalIdxCount)
        {
            ...
            fr.IndexBuffer = _device.CreateCommittedResource(
                props,
                HeapFlags.None,
                desc,
                ResourceStates.GenericRead
        ...
        var vtx_dst = (ImDrawVert*)vtx_resource;
        var idx_dst = (ushort*)idx_resource;
        for (var n = 0; n < draw_data.CmdListsCount; n++)
        {
            ...
            Buffer.MemoryCopy(cmd_list.VtxBuffer.Data, vtx_dst, len, len);

            len = cmd_list.IdxBuffer.Size * sizeof(ushort);
            Buffer.MemoryCopy(cmd_list.IdxBuffer.Data, idx_dst, len, len);
        ...
        for (var n = 0; n < draw_data.CmdListsCount; n++)
        {
            ...
            for (var cmd_i = 0; cmd_i < cmd_list.CmdBuffer.Size; cmd_i++)
            {
                ...
                else
                {
                    ...
                    texture_handle.Ptr = (long)pcmd.TextureId.Handle;
    ...
    public static unsafe void CreateFontsTexture()
    {
        ...
        // Upload texture to graphics system
        {
            ...
            var pTexture = _device.CreateCommittedResource(
                props,
                HeapFlags.None,
                desc,
            ...
            var uploadPitch = (uint)(
                (width * 4 + D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u)
                & ~(D3D12_TEXTURE_DATA_PITCH_ALIGNMENT - 1u)
            );
            var uploadSize = (uint)(height * uploadPitch);
            desc.Dimension = ResourceDimension.Buffer;
            desc.Alignment = 0;
            ...
            Range range = new() { Begin = 0, End = uploadSize };

            var mapped = (nuint)(nint)uploadBuffer.Map(0, range);

            var nuintHeight = (nuint)height;
            var nuintWidth = (nuint)width;
        ...
        io.Fonts.SetTexID(new ImTextureID((nint)hFontSrvGpuDescHandle.Ptr));
    }
    ...
    public static bool CreateDeviceObjects()
    {
        ...
        var param = new RootParameter[2];

        var param0 = new RootParameter(
        ...
        staticSampler.Filter = Filter.ComparisonMinMagMipLinear;
        staticSampler.AddressU = TextureAddressMode.Wrap;
        staticSampler.AddressV = TextureAddressMode.Wrap;
        staticSampler.AddressW = TextureAddressMode.Wrap;
        staticSampler.MipLODBias = 0.0f;
        staticSampler.MaxAnisotropy = 0;
        staticSampler.ComparisonFunc = Comparison.Always;
        ...
        pRootSignature = _device.CreateRootSignature(
            new DataPointer(blob.BufferPointer, blob.BufferSize)
        );
        ...
        {
            var blendStateDescription = psoDesc.BlendState;
            ...
            blendStateDescription.RenderTarget[0].DestinationAlphaBlend =
        ...
        // Create the rasterizer state
        {
            ...
            var rasterizerStateDescription = psoDesc.RasterizerState;
        ...
        // Create depth-stencil State
        {
            ...
            depthStencilState.FrontFace.FailOperation =
                depthStencilState.FrontFace.FailOperation =
    ...
    public static void InvalidateDeviceObjects()
    {
        ...
        io.Fonts.SetTexID(ImTextureID.Null); // We copied bd->pFontTextureView to io.Fonts->TexID so let's clear that as well.
    ...
    internal static unsafe void Init(
        ...
        Format rtv_format,
        void* cbv_srv_heap,
        CpuDescriptorHandle font_srv_cpu_desc_handle,
        GpuDescriptorHandle font_srv_gpu_desc_handle
    )
    {
        if (_device != null)
        {
        ...
        for (var i = 0; i < num_frames_in_flight; i++)
        {
    ...
    private unsafe delegate void ImDrawUserCallBack(ImDrawListPtr a, ImDrawCmd* b);
}
